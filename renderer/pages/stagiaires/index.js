      ...
      <div>
        ...
        <div className="bg-white rounded-lg shadow">
          ...
          <div className="lg:hidden divide-y divide-gray-200">
            {filteredStagiaires.length === 0 ? (
              <div className="px-4 py-8 text-center text-gray-500">Aucun stagiaire trouvé</div>
            ) : (
              filteredStagiaires.map((stagiaire) => (
                <div key={stagiaire.id} className="p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-gray-900">{stagiaire.nom} {stagiaire.prenom}</span>
                    <span
                  ...
                  <div className="text-sm text-gray-500">
                    {stagiaire.telephone || '-'} {stagiaire.email ? `· ${stagiaire.email}` : ''}
                  </div>
                  <div className="text-sm text-gray-500">{stagiaire._count?.inscriptions || 0} inscriptions</div>
                  <div className="flex justify-end gap-4 pt-2 border-t text-sm font-medium">
                    <Link href={`/stagiaires/${stagiaire.id}`} className="text-blue-600 hover:text-blue-900">Voir</Link>
                    <Link href={`/stagiaires/${stagiaire.id}/modifier`} className="text-green-600 hover:text-green-900">Modifier</Link>
                    <button onClick={() => deleteStagiaire(stagiaire.id)} className="text-red-600 hover:text-red-900">Supprimer</button>
                  </div>
                </div>
              ))
            )}
